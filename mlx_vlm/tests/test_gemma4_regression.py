...
class TestGemma4Regression(unittest.TestCase):
    ...
    def test_load_mlx_format_detects_metadata_across_shards(self):
        """
        Verify that MLX metadata on any safetensors shard selects the MLX path.
        """
        with tempfile.TemporaryDirectory() as tmp_dir:
            tmp_path = Path(tmp_dir)
            weights = self._create_minimal_gemma4_checkpoint(tmp_path)
            ...
            # The first shard intentionally has no metadata. A loader that only
            # checks the first shard would incorrectly take the non-MLX path.
            mx.save_safetensors(
            ...
            with patch(
                ...
                side_effect=AssertionError("MLX shards should skip sanitization"),
            ):
            ...
            self.assertNotIn(
                "language_model.model.layers.2.self_attn.k_proj.weight",
                loaded_weight_keys,
                "Unused shared KV weight should have been filtered from sharded MLX checkpoint",
            )
